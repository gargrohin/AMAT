...
# build network
z_dim = 100
mnist_dim = dataset.data.size(1) * dataset.data.size(2)
...
for d in range(n_dis):
    ...
    multiD_optim.append(optim.Adam(multiD[d].parameters(), lr = lr/5, betas = (0.5, 0.999)))
# loss
...
def D_train(x_real):
    ...
    if n_dis > 2:
        mask = torch.zeros((x_real.size()[0], n_dis-2)).to(device)
    # mask2 = torch.zeros((x_real.size()[0], n_dis)).to(device)

    if n_dis > 2:
        for i in range(mask.size()[0]):
            random_checker = np.random.randint(0,10)
            if random_checker > 8:
                index = np.random.randint(0,n_dis-2)
                mask[i][index] = 1.0
            else:
                mask[i][ind[i]] = 1.0
    else:
...
experiment = comet_ml.Experiment(project_name="splitmnist_multidcgan")

exp_parameters = {
    "data": "mnist_64x64",
    "model": "64xDConv, gen_tanh",
    "opt_gen": "Adam_lr_0.0002, (0.5,0.999)",
    "opt_dis": "Adam_lr_0.00002, (0.5,.999)",
    "n_dis": "2+2", 
    "z_dim": 100,
    "n_critic": 1,
...
for epoch in range(1, n_epoch+1):
    ...
    print('\n[%d/%d]: loss_d: %.3f, acc_d: %.3f, loss_g: %.3f' % (
            (epoch), n_epoch, torch.mean(torch.FloatTensor(D_losses)), 0.0, torch.mean(torch.FloatTensor(G_losses))))
