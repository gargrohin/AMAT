...
dataset_ordered = []
for i in range(10):
  dataset_ordered.append(train_dataset.data[train_dataset.targets==i])
...
for i in range(10):
  if i in remove:
    continue
  if flag:
    dataset_new = dataset_ordered[i]
    flag = False
  else:
    dataset_new = torch.cat((dataset_new, dataset_ordered[i]))
...
class toy_dataset(Dataset):
  ...
  def __getitem__(self,idx):
    ...
    return sample
trainset = toy_dataset(dataset_new, transform)
...
experiment = comet_ml.Experiment(project_name="mnist_none_dcgan")
