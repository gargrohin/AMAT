...
for d in range(n_dis):
    ...
    multiD_optim.append(optim.Adam(multiD[d].parameters(), lr = lr, betas = (0.5, 0.999)))
# loss
...
def D_train(x):
    ...
    with torch.no_grad():
        G.eval()
        z = Variable(torch.rand(1, z_dim).cuda())
        fake = G(z)
        for dis_index in range(n_dis):
            fake_score = torch.mean(multiD[dis_index](fake)).item()
            fake_vals.append(fake_score)
    
    fake_vals = np.array(fake_vals)
    dis_index = np.argmin(fake_vals)
    # random_checker = np.random.randint(0,10)
    # if random_checker >8:
    # dis_index = np.random.randint(0,n_dis)
    #=======================Train the discriminator=======================#
    G.train()
    for p in multiD[dis_index].parameters():
        p.reauires_grad = True
    multiD[dis_index].zero_grad()

    # train discriminator on real
    x_real, y_real = x, torch.ones(x.size()[0], 1)
    x_real, y_real = Variable(x_real.to(device)), Variable(y_real.to(device))

    D_output_real = multiD[dis_index](x_real)
    ...
    D_output_fake = multiD[dis_index](x_fake)
    ...
    return  D_loss.data.item(), D_acc, dis_index
...
experiment = comet_ml.Experiment(project_name="mnist_dcgan_multiD_uselosses_1")
...
exp_parameters = {
    ...
    "dis_landscape": 0,
    "try": 3,
    # "D update": "ndis 1 try whatever argmin over 1 datapoint score, but update over bs",
    "D update": "argmin over 1 datapoint score, but update over bs",
    }
